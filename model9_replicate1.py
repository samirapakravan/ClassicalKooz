...
demography.set_migration_rate(source="Ghost", dest="pop2", rate=0.5)
...
demography.set_migration_rate(source="Ghost", dest="pop1", rate=0.5)
...
ts = msprime.sim_ancestry(sequence_length=100, samples={"pop1": 10, "pop2": 10, "Ghost":10}, demography=demography, random_seed=2148)

#Simulate mutations along the tree sequence simulated by msprime above
mts = msprime.sim_mutations(ts, rate=0.01, random_seed=2148)
...
with open("model9_replicate1.vcf","w") as vcf_file:
