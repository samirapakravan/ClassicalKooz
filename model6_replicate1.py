...
demography.add_population_split(time=18500, derived=["pop1", "NA2"], ancestral="NA1")
demography.add_population_split(time=1850, derived=["Ghost", "pop2"], ancestral="NA2")

demography.set_migration_rate(source="pop2", dest="Ghost", rate=0.1)
demography.set_migration_rate(source="Ghost", dest="pop2", rate=0.1)
demography.set_migration_rate(source="pop1", dest="pop2", rate=0.1)
demography.set_migration_rate(source="pop2", dest="pop1", rate=0.1)
demography.set_migration_rate(source="pop1", dest="Ghost", rate=0.1)
demography.set_migration_rate(source="Ghost", dest="pop1", rate=0.1)
...
ts = msprime.sim_ancestry(sequence_length=100, samples={"pop1": 10, "pop2": 10, "Ghost":10}, demography=demography, random_seed=7760)

#Simulate mutations along the tree sequence simulated by msprime above
mts = msprime.sim_mutations(ts, rate=0.01, random_seed=7760)
...
with open("model6_replicate1.vcf","w") as vcf_file:
